...
object spark23_sortByKey {
  def main(args: Array[String]): Unit = {
    ...
    //sortByKey
    //升序
    val sortRDD01: RDD[(Double, String)] = result.map(a=>(a._2,a._1)).sortByKey(true)
    println(sortRDD01.collect().mkString(","))

    //降序
    val sortRDD02: RDD[(Double, String)] = result.map(a=>(a._2,a._1)).sortByKey(false)
    println(sortRDD02.collect().mkString(","))
